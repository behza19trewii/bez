...
یک اسکریپت تک‌فایلی پیشرفته برای بک‌تست استراتژی SMA crossover
با خروجی‌های کامل، بهینه‌سازی پارامتر، شاخص‌های عملکرد و نمودارهای ذخیره‌شدنی.
...
# ------------------------- Utilities -------------------------
def load_data(path_csv='data.csv', path_json='data.json'):
    """
    تلاش می‌کند ابتدا CSV و سپس JSON را بارگذاری کند.
    اگر هیچ فایلی نبود، دادهٔ ساختگی تولید می‌کند.
    خروجی: DataFrame indexed by Date with at least 'Close' column.
    """
    if os.path.exists(path_csv):
        ...
        if 'Date' in df.columns:
            ...
            # فرض می‌کنیم ستون اول تاریخ است
        ...
        if 'Close' not in df.columns:
            if df.shape[1] == 1:
                ...
                raise ValueError("CSV found but no 'Close' column. Provide at least Date and Close.")
        ...
        if 'Close' not in df.columns:
            if df.shape[1] == 1:
                ...
                raise ValueError("JSON found but no 'Close' column. Provide at least Date and Close.")
        return df[['Close']].copy()
    else:
...
def backtest_sma(df, ma_short=10, ma_long=None, initial_capital=10000.0, position_size_pct=0.5,
                 commission=0.0, slippage=0.0, exit_on_ma_cross=True,
                 take_profit_pct=None, stop_loss_pct=None, max_holding_days=None,
                 use_trailing_atr=True, atr_mult=3.0, atr_period=14):
    """
    پیاده‌سازی بک‌تست:
    - ورود: وقتی SMA_short امروز بالاتر از SMA_long شد و دیروز <= بود.
    - خروج: بصورت ترکیبی از (MA-cross down) یا TP/SL یا Trailing ATR یا حداکثر مدت نگهداری.
    ...
    for i in range(1, len(data)):
        ...
        sma_s_today = data[f'SMA_{ma_short}'].iloc[i]
        sma_l_today = data[f'SMA_{ma_long}'].iloc[i]
        sma_s_yest = data[f'SMA_{ma_short}'].iloc[i - 1]
        sma_l_yest = data[f'SMA_{ma_long}'].iloc[i - 1]
        ...
        if in_pos:
            ...
            tp_hit = False; sl_hit = False; ts_hit = False; max_hold_hit = False
            ...
            if exit_on_ma_cross and exit_signal_ma:
                will_exit = True; reason = 'ma_cross'
            if tp_hit:
                will_exit = True; reason = 'take_profit'
            if sl_hit:
                will_exit = True; reason = 'stop_loss'
            if ts_hit:
                will_exit = True; reason = 'trailing_atr'
            if max_hold_hit:
                will_exit = True; reason = 'max_hold'
            ...
            if will_exit:
                ...
                trade['pct_return'] = (trade['exit_price'] / trade['entry_price'] - 1) * 100
...
def optimize_grid(df, ma_shorts=[5, 10, 15, 20], tp_values=[None, 0.03, 0.05], **kwargs):
    ...
    for ma in ma_shorts:
        for tp in tp_values:
            ...
            pf = profit_factor(trades) if not trades.empty else np.nan
            results.append({
                'ma_short': ma,
...
# ------------------------- Main (config + run) -------------------------
def main():
    # ---------- Default user config (می‌تونی اینها را تغییر بدهی) ----------
...
initial_capital=INITIAL_CAPITAL,
    ...
    win_trades = bt_trades[bt_trades['pct_return'] > 0]
    loss_trades = bt_trades[bt_trades['pct_return'] <= 0]
